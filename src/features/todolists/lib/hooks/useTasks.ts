...
export const useTasks = (todolist: DomainTodolist) => {
  ...
  const { data, isLoading, isFetching, error } = useGetTasksQuery({ todolistId: id, args: { page } })
  ...
  return { tasks, isLoading, isFetching, page, setPage, totalCount: data?.totalCount, error }
}
